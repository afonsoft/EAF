...
{
    public class FeatureValueStoreTests : EafMiddlewareTestBase
    {
        ...
        public void GetTenantFeatureCacheItem_ShouldEnableFilterMayHaveTenant_Test()
        {
            ...
            {
                _tenantFeatureRepository.Insert(new TenantFeatureSetting(tenant.Id, AppFeaturesSample.SimpleBooleanFeature, "true"));
                _unitOfWorkManager.Current.SaveChanges();

                // Assert (before disable filter)
                _featureValueStore.GetValueOrNull(tenant.Id, AppFeaturesSample.SimpleBooleanFeature).ShouldBe("true");
                _featureValueStore.GetValueOrNull(tenant2.Id, AppFeaturesSample.SimpleBooleanFeature).ShouldBeNull();
        ...
        public async Task GetTenantFeatureCacheItemAsync_ShouldEnableFilterMayHaveTenant_Test()
        {
            ...
            using (var uow = _unitOfWorkManager.Begin())
            {
                await _tenantFeatureRepository.InsertAsync(new TenantFeatureSetting(tenant.Id, AppFeaturesSample.SimpleBooleanFeature, "true"));
                await _unitOfWorkManager.Current.SaveChangesAsync();

                // Assert (before disable filter)
                (await _featureValueStore.GetValueOrNullAsync(tenant.Id, AppFeaturesSample.SimpleBooleanFeature)).ShouldBe("true");
                (await _featureValueStore.GetValueOrNullAsync(tenant2.Id, AppFeaturesSample.SimpleBooleanFeature)).ShouldBeNull();
