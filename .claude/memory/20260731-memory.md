...
## Harness Migration (2026-07-31)
- Migrated EAF agent harness from `.agents/`, `rules/` and `AGENTS.md` to the native Claude Code structure (`.claude/`).
- Decisions:
  - `CLAUDE.md` rewritten as the single source of truth; `AGENTS.md` removed.
  - `.agents/skills/` moved to `.claude/skills/{slug}/` preserving references/templates/scripts.
  - `.agents/RULES.md`, `CONTEXT.md`, `TOOLS.md`, `WORKFLOWS.md` and `README.md` consolidated into `CLAUDE.md`, `.claude/rules/global-rules.md` and `.claude/knowledge/`.
  - `rules/*.instructions.md` converted to `.claude/rules/*.md` with `paths:` instead of the old glob syntax.
  - `.claudeignore`, `.devinignore`, `.windsurfignore` replaced by `permissions.deny` in `.claude/settings.json`.
  - `.agents/MEMORY.md` historical entries moved to `.claude/memory/20260731-memory.md`; `.claude/memory/memory.md` seeded with current state.
  - `opencode.json` left untouched (OpenCode server config, outside Claude harness scope).
- Branch: `feature/claude-20260801-bootstrap-claude-harness`.
- Verification: legacy inventory checks empty; target structure and memory validation passed.

## Harness Migration merged (2026-07-31)
- PR #279 merged into `main` at commit `f808478`.
- Native Claude Code harness now lives in `CLAUDE.md` + `.claude/`.
- Validation re-run after merge: legacy artifacts absent, target structure present, `settings.json` valid JSON.
- Next session must read `CLAUDE.md`, `global-rules.md` and `.claude/memory/memory.md` first.
