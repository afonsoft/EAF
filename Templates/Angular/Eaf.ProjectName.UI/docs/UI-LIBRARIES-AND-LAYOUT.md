# Bibliotecas e Padrões de Layout — EAF Angular UI

Documento de referência sobre as bibliotecas de UI, grid, tabelas, formulários e padrões visuais usados no template Angular do EAF (`Templates/Angular/Eaf.ProjectName.UI`).
...
| Node.js | 20.20.0 (`nvm use 20`) | Runtime de build |
...
| PrimeNG | 17.17.0 (resolvido para 17.18.15) | Tabelas, paginação, editor, autocomplete, upload, árvore |
...
<tabset class="tab-container tabbable-line" *ngIf="settings">
  ...
  <tab heading="{{ 'Security' | localize }}" customClass="m-tabs__item">
    <!-- conteúdo -->
  </tab>
</tabset>
```
...
Arquivos principais configurados em `angular.json`:
...
Cada tema possui seu `style.bundle.css` e `customize.css` em `src/assets/common/styles/themes/<nome>/`:
...
- `src/app/admin/settings/settings.component.html` — exemplo de configurações com abas
- `src/app/admin/ui-customization/default-theme-ui-settings.component.html` — exemplo de form Metronic por aba
- `src/app/admin/users/create-or-edit-user-modal.component.html` — exemplo de modal com abas
- `src/app/admin/payments/payments.component.html` — exemplo de listagem PrimeNG
- `src/app/admin/payments/payment-gateway-settings-modal.component.html` — modal de configuração de gateways
- `src/app/shared/components/status-badge/status-badge.component.ts` — componente de badge
- `src/app/shared/components/empty-state/empty-state.component.ts` — componente de estado vazio
- `src/assets/common/styles/styles.css` — estilos customizados EAF
- `src/assets/common/styles/themes/*/style.bundle.css` — bundles de tema
