...
describe('FeaturesComponent', () => {
  ...
  beforeEach(() => {
    ...
    TestBed.configureTestingModule({
      declarations: [FeaturesComponent, MockLocalizePipe],
      providers: [
  ...
  it('should filter features by name or display name', () => {
    component.features = [
      { name: 'FeatureA', displayName: 'Alpha' },
      { name: 'FeatureB', displayName: 'Beta' },
    ] as any;
    component.filterText = 'alpha';
    component.filterFeatures();

    expect(component.filteredFeatures.length).toBe(1);
    expect(component.filteredFeatures[0].name).toBe('FeatureA');
  });
});
